  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-3 gap-12 mt-4'>
      {props.posts.map((post: Post) => (
        <div key={post.id} className='w-full sm:w-40 md:w-60 lg:w-80 h-full bg-white'>
          <Link href={`/blog/${post.id}`} className='block cursor-pointer border-2 rounded-md neo-shadow focus:shadow-none focus:translate-x-1 focus:translate-y-1 transform transition-shadow duration-100'>
            <article className='w-full h-full'>
              <figure className='w-full h-30 md:h-40 lg:h-72 border-b-2'>
                {
                  post.imgURL ? (
                    <Image src={post.imgURL} alt="thumbnail" layout="responsive" width={500} height={300} className='w-full h-full object-cover' />
                  ) : (
                    <Image src="/article-placeholder.png" alt="thumbnail" layout="responsive" width={500} height={300} className='w-full h-full object-cover' />
                  )
                }
              </figure>
            </article>
